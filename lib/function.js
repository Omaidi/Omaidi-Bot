...
const SERVER_ZIP = 'https://github.com/Omaidi/hitori/archive/refs/heads/master.zip';
const VERSION_URL = 'https://raw.githubusercontent.com/Omaidi/hitori/master/package.json';
...
async function updateSettings({
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (apikey) content = content.replace(/(global\.APIKeys\s*=\s*\{[\s\S]*?'https:\/\/api\.Omaidi\.biz\.id'\s*:\s*')[^']*(')/, `$1${apikey}$2`);
